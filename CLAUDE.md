...
This project defines Kubernetes CRDs and API types for BGP routing and virtual networking. It is **API-only** — no controllers, no binaries, no runtime. Implementations consume these APIs; this repo just defines the contract.

Module: `go.datum.net/network`
...
| `vpc.network.datumapis.com` | `v1alpha1` | VPC, VPCAttachment                                              |

Source lives in `api/v1alpha1/` and `api/vpc/v1alpha1/`. Each resource has its own `*_types.go` file; shared types (RouterTarget, AddressFamily, etc.) live in `shared_types.go`.
