...
> This project is a Go module that defines Kubernetes CRDs and API types for BGP routing and virtual networking. It is consumed as a library by external controller implementations; this repository contains no runtime code.
...
3. [Module / Package Reference](#module--package-reference)
   - [api/v1alpha1](#apibgpv1alpha1)
4. [API Resource Model](#api-resource-model)
   - [BGP Group](#bgp-group-bgpsdatumnet)
...
This module defines the Kubernetes Custom Resource Definitions (CRDs) and Go API types for BGP routing:

- **BGP routing** (`network.datumapis.com`) — abstractions for BGP routers, peers, advertisements, policies, and L2VPN/EVPN VRF instances

The module is a **library**, not a binary. External controllers import `go.datum.net/network` to register these types with their scheme and reconcile the resources. It ships only the type definitions, validation rules, CRD YAML, and their tests. There are no controllers, no HTTP servers, no goroutines, and no persistent state within this module.
...
```
├── api/
│   └── v1alpha1/              # network.datumapis.com/v1alpha1 — 5 CRD types + generated deepcopy
├── config/
│   ├── crd/                   # Generated CRD YAML (controller-gen output — never edit directly)
│   └── samples/               # Example resource manifests
├── docs/
│   ├── api/                   # Human-readable field reference (bgp.md)
...
├── go.mod                     # Module: go.datum.net/network, Go 1.26
├── go.sum
...
**Purpose:** Defines all Go types, validation rules, and scheme registration for the `network.datumapis.com/v1alpha1` API group. Contains five CRD resources and their shared supporting types.
...
### BGP Group (`network.datumapis.com`)
...
**BGPRouter** — `bgprouters.network.datumapis.com`, shortName `bgpr`
...
**BGPPeer** — `bgppeers.network.datumapis.com`, shortName `bgppr`
...
**BGPAdvertisement** — `bgpadvertisements.network.datumapis.com`, shortName `bgpadv`
...
**BGPPolicy** — `bgppolicies.network.datumapis.com`, shortName `bgpp`
...
**BGPVRFInstance** — `bgpvrfinstances.network.datumapis.com`, shortName `bgpvrf`
...
```go
import bgpv1alpha1 "go.datum.net/network/api/v1alpha1"

scheme.AddToScheme(bgpv1alpha1.AddToScheme)
```

`AddToScheme` is produced by `sigs.k8s.io/controller-runtime/pkg/scheme.Builder` and registered via `init()` in `groupversion_info.go`.
...
External controller (watches the API group via controller-runtime)
  imports go.datum.net/network API types to decode resources
        │
        ▼
...
- Location: `api/v1alpha1/*_types_test.go`
...
- Run: `GOOS=linux go test ./api/v1alpha1/... -v`  or `task test:unit`
