...
> This project is a Go module defining Kubernetes CRDs and API types for BGP routing and virtual networking. It is API-only — no controllers, no runtime code.
...
```
api/
  v1alpha1/             ← network.datumapis.com API group
...
Scopes in use: `api`, `bgp`, `e2e`, `docs`, `fmt`, `proto`, `controller`.
...
- Module path: `go.datum.net/network`
- Go version: 1.26
- Package name equals the directory's last path segment (`v1alpha1`).
- One package per API group version. All types for `network.datumapis.com/v1alpha1` live in `api/v1alpha1/`; one `_types.go` file per CRD.
...
|-------------------------|-----------------------------------------------|------------------------------------------------|
| CRD resource            | `PascalCase` prefixed with group abbreviation | `BGPRouter`, `BGPPeer`, `BGPAdvertisement`     |
...
- Commit messages follow Conventional Commits: `type(scope): message`. Scopes are `api`, `bgp`, `e2e`, `docs`.
