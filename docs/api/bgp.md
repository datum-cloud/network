...
| Group | Purpose |
|-------|---------|
| `network.datumapis.com/v1alpha1` | BGP routing context, sessions, advertisements, and policies |
...
`ConditionTypeAccepted` in `api/v1alpha1/peer_types.go`.
...
in `api/v1alpha1/peer_types.go`. Call it whenever `sessionState` changes:
...
spec:
  routerSelector:
    matchLabels:
      network.datumapis.com/role: tenant
  direction: import
  terms:
    - sequence: 10
      match:
