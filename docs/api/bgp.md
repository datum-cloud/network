...
| Field | Description |
| --- | --- |
| `IPv6` | EgressAddressFamilyIPv6 is reached by NAT66 through the shard's IPv6<br />address.<br /> |
| `IPv4` | EgressAddressFamilyIPv4 is reached by NAT64 through the shard's IPv4<br />address.<br /> |
...
| `families` _[EgressAddressFamily](#egressaddressfamily) array_ | Families are the destination address families the attachment's network<br />declared, so the shard on the node is one that translates them. |  | Enum: [IPv6 IPv4] <br />MaxItems: 2 <br />MinItems: 1 <br /> |
