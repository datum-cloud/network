...
with the project managing two BGP planes.
...
```bash
kubectl get crds | grep datum.net
```

You should see:

```
bgpadvertisements.network.datumapis.com
bgppeers.network.datumapis.com
bgppolicies.network.datumapis.com
bgprouters.network.datumapis.com
bgpvrfinstances.network.datumapis.com
```
...
metadata:
  ...
  namespace: default
  labels:
    network.datumapis.com/role: fabric
spec:
  targetRef:
    kind: Node
    name: node-1
